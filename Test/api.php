<?php
    ...
    switch ($input->action){
        ...
        case "login":
            $cursor=$mysqldb->prepare("SELECT `username`, `Password`, `email`, `last_browser`, `last_ipaddr`, `avatar` FROM `users` WHERE email=:email");
            $cursor->execute(array(':email'=> $input->email));
            if (!empty($cursor)){
                return_success(array("success" => true, "msg" =>json_encode($cursor->fetchall())));
            }
            else{
                die(json_encode(array("msg" =>json_encode("Invalid Username or Password"))));
                //return_error(array("success" => true, "msg" =>json_encode("Invalid Username or Password")));
            }
        ...
        case "Forgot_Password":
            ...
            if(mail($to, $subject, $message, $headers)){
                return_success(array("success" => true, "msg" =>json_encode("Your Password has been sent to your email id")));
            }
            else{
                return_error(array("success" => false, "msg" =>json_encode("Failed to Recover your password, try again")));
            }
